...
const productIndex = Number(editIndex);
...
if (!isValidIndex) {
    ...
    Toastify({
        ...
        style: {
            background: "linear-gradient(135deg, rgba(170, 55, 55, 0.82), rgba(220, 85, 85, 0.65))",
...
if (isValidIndex) {
    ...
    document
        ...
        .forEach((input) => {
            ...
            input.checked =
                input.value === oldProduct.status;

        });


    document
        .querySelectorAll('input[name="colors"]')
        .forEach((input) => {

            input.checked =
                Array.isArray(oldProduct.colors) &&
                oldProduct.colors.includes(input.value);
