...
function loadQuestion() {
    ...
    question.options.forEach((option, index) => {
        ...
        input.addEventListener("change", () => {
            if (!submitted) answers[currentQuestion] = Number(input.value);
        });
...
function calculateResult() {
    ...
    questions.forEach((question, index) => {
        if (answers[index] === null) skipped++;
        else if (answers[index] === question.answer) right++;
        else wrong++;
    });
...
yesBtn.addEventListener("click", submitQuiz);
...
resultModal.addEventListener("click", (event) => {
    if (event.target === resultModal) {
        // Keep the result modal open until Done is selected.
    }
});
